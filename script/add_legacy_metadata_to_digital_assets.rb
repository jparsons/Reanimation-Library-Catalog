...
xml_document.search("//row").each {|node|
  ...
  image_ids.each_with_index do |image_name_node, i|
    ...
    if digital_asset
      digital_asset.update_attribute(:color, colors[i].inner_html())
      puts colors[i].inner_html()
    end
  end
}
...
xml_document.search("//row").each {|node|
  ...
  image_ids.each_with_index do |image_name_node, i|
    ...
    if digital_asset
      digital_asset.update_attribute(:image_type, image_types[i].inner_html())
      puts image_types[i].inner_html()
    end
  end
}
...
xml_document.search("//row").each {|node|
  ...
  subjects.each_with_index do |subject, i|
    ...
    digital_asset_subject = DigitalAssetSubject.find_or_create_by_name_and_authority(subject.inner_html, authorities[i].inner_html)
  end
}
